...
async def create_engagement(req: CreateEngagementRequest):
    """Create a new engagement keyed on (acquirer_entity_id, target_entity_id).

    tenant_id is shared context for triple lookups within the same AOS instance.
    Runs entity-level contract check on both entities before creation.
    """
    if not req.tenant_id or not req.acquirer_entity_id or not req.target_entity_id:
        raise HTTPException(
            status_code=422,
            detail="tenant_id, acquirer_entity_id, and target_entity_id are all required.",
        )

    if req.acquirer_entity_id == req.target_entity_id:
        raise HTTPException(
            status_code=422,
            detail=f"acquirer_entity_id and target_entity_id must be distinct. "
                   f"Both are: {req.acquirer_entity_id}",
        )

    acq_contract = check_entity_contract(req.tenant_id, req.acquirer_entity_id)
    tgt_contract = check_entity_contract(req.tenant_id, req.target_entity_id)
    ...
    if failures:
        raise HTTPException(
            status_code=422,
            detail={
                "error": "ENTITY_CONTRACT_FAILED",
                "message": "One or both entities failed the contract check.",
                "diagnostics": failures,
            },
        )

    try:
        result = engagement_store.create_engagement(
