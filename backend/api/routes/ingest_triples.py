...
def ingest_triples(
    ...
    if run_exists and not replace and not append:
        raise HTTPException(
            ...
            detail={
                ...
                "message": f"pipeline_run_id {req.run_id} already has triples in the store. "
                           "Use ?replace=true to deactivate old triples and re-ingest, "
                           "or ?append=true to add more triples to this run.",
                "convergence_ingest_id": req.run_id,
            },
        )

    if run_exists and replace:
        logger.info(
            f"[ingest-triples] replace=true for existing pipeline_run_id={req.run_id}; "
    ...
    try:
        ...
        logger.error(
            ...
            f"pipeline_run_id={req.run_id}, tenant_id={req.tenant_id}, "
    ...
    run_name = f"{eng.short_name}-{req.run_id[:4]}"

    logger.info(
        f"[ingest-triples] Ingested {count} triples for pipeline_run_id={req.run_id}, "
