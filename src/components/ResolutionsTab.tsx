...
export default function ResolutionsTab({ engagementId }: { engagementId: string }) {
  ...
  const [resolveResult, setResolveResult] = useState<{ domains_resolved: number; stats: Record<string, number> } | null>(null);
  ...
  const runResolver = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await apiFetch<{ domains_resolved: number; stats: Record<string, number> }>(
        `/engagements/${engagementId}/resolve`,
  ...
  return (
    <div ref={containerRef} className="space-y-4" tabIndex={-1}>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={runResolver}
            disabled={resolving}
            className="inline-flex items-center gap-1.5 px-2.5 py-1 text-xs bg-cyan-600/20 text-cyan-400 border border-cyan-500/30 rounded-lg hover:bg-cyan-600/30 disabled:opacity-50 transition-colors"
            data-testid="run-resolver-btn"
          >
            {resolving ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Play className="w-3.5 h-3.5" />}
            {resolving ? 'Resolving...' : 'Run Resolver'}
          </button>
          <button
            onClick={fetchData}
      ...
      {resolveResult && (
        <div className="bg-green-950/40 border border-green-800/50 rounded-lg px-4 py-3 text-green-200 text-sm" data-testid="resolve-result">
          Resolver completed: {resolveResult.domains_resolved} domain{resolveResult.domains_resolved !== 1 ? 's' : ''} resolved
          {' \u2014 '}
          {Object.entries(resolveResult.stats).map(([k, v]) => `${v} ${k.replace('_', ' ')}`).join(', ')}
        </div>
      )}
      ...
      {resolutions?.domains.length === 0 && (
        <div className="text-center py-8">
          <p className="text-gray-500 mb-3">No resolver decisions found.</p>
          <button
            onClick={runResolver}
            disabled={resolving}
            className="inline-flex items-center gap-1.5 px-4 py-2 bg-cyan-600 hover:bg-cyan-500 disabled:bg-gray-600 disabled:cursor-not-allowed text-white text-sm font-medium rounded-lg transition-colors"
            data-testid="run-resolver-empty-btn"
          >
            {resolving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Play className="w-4 h-4" />}
            {resolving ? 'Running Resolver...' : 'Run Resolver'}
          </button>
        </div>
      )}
