...
|------|-------------|----------|-----------|
| engine/entity_resolution.py | conflict_detection.py, entities.py route, maestra.py route, query.py | **Move** | All importers are ME-specific except query.py and maestra.py route (decoupled in Phase 3) |
...
| File | Imported By | Decision | Rationale |
|------|-------------|----------|-----------|
| engine/_engine_cache.py | dashboards.py, maestra.py (lines 666-682), reports.py | **Move** | Caches cross_sell, ebitda_bridge, qoe — all ME. maestra.py decoupled in Phase 3. |
| engine/dashboards.py | maestra.py (line 934) | **Move** | Imports get_active_engagement + _engine_cache. maestra.py decoupled in Phase 3. |
...
| backend/api/routes/maestra.py:34 | `entity_resolution.get_entity_store` | Phase 3: Replace with HTTP call to convergence |
